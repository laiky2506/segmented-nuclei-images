...
#%%
#1.2. Load the images
image_dir = os.path.join(file_directory,'train\inputs')
...
#1.3. Load the masks
mask_dir = os.path.join(file_directory,'train\masks')
for mask_file in os.listdir(mask_dir):
    mask = cv2.imread(os.path.join(mask_dir,mask_file),cv2.IMREAD_GRAYSCALE)
    mask = cv2.resize(mask,(128,128))
    masks.append(mask)

#1.4. Load the wild images
wild_image_dir = os.path.join(file_directory,'test\inputs')
for wild_image_file in os.listdir(wild_image_dir):
    img = cv2.imread(os.path.join(wild_image_dir,wild_image_file))
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = cv2.resize(img,(128,128))
    wild_images.append(img)
    
#1.3. Load the wild masks
wild_mask_dir = os.path.join(file_directory,'test\masks')
for wild_mask_file in os.listdir(wild_mask_dir):
    mask = cv2.imread(os.path.join(wild_mask_dir,wild_mask_file),cv2.IMREAD_GRAYSCALE)
    mask = cv2.resize(mask,(128,128))
    wild_masks.append(mask)
...
wild_images_np = np.array(wild_images)
wild_masks_np = np.array(wild_masks)
...
wild_masks_np_exp = np.expand_dims(wild_masks_np,axis=-1)
...
converted_wild_masks = np.round(wild_masks_np_exp/255)
converted_wild_masks = 1 - converted_wild_masks

#%%
#2.3. Normalize the images
converted_images = images_np / 255.0
converted_wild_images = wild_images_np / 255.0
...
wild_x = tf.data.Dataset.from_tensor_slices(converted_wild_images)
wild_y = tf.data.Dataset.from_tensor_slices(converted_wild_masks)

#%%
#2.6. Zip tensor slice into dataset
train = tf.data.Dataset.zip((train_x,train_y))
validation = tf.data.Dataset.zip((test_x,test_y))
test = tf.data.Dataset.zip((wild_x,wild_y))
